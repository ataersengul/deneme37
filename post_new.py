...
POST_AND_COMMENTS_FILE_HEADERS = ['PostText','PosterName','PostDate','CommentDate','CommentContact','CommentDetails','CommentMedia']
...
driver.get("https://www.facebook.com/groups/5658649590/permalink/10156440500484591/")
groupId = "5658649590"
...
time.sleep(10)
elementsWallPostDiv = driver.find_elements_by_xpath("//*[starts-with(@id, 'group_mall_5658649590')]")
elementsImages = driver.find_elements_by_xpath("//*[starts-with(@id, 'group_mall_5658649590')]//*[starts-with(@id, 'u_')]//*[starts-with(@class,'uiScaledImageContainer')]//img")


print(str(len(elementsWallPostDiv)) + " Posts will be saved")
print(str(len(elementsImages)) + " Media will be saved")

main_element_post = "//*[starts-with(@id, 'group_mall_5658649590')]"


try:
    for wallPost in elementsWallPostDiv:
        wallPostId = "" #wallPost.get_property("id")
        WallPostTextXpathWithId = "//*[starts-with(@id, 'u_')]//p"
        ...
        try:
            ...
            try:
                LastCommentPostDate = driver.find_element_by_xpath(FormattedWallPostLastCommentDateXpathWithId).get_attribute("data-utime")

            except:
                LastCommentPostDate = driver.find_element_by_xpath(FormattedWallPostDateXpathWithId).get_attribute("data-utime")
            ...
            try:
                ...
                if not os.path.exists("Posts"):
                    os.makedirs("Posts")
                with open("Posts\\" + formattedLastCommentPostDate + "_" + FormattedwallPostID + '.csv', 'w') as csvfile:
                    fieldnames = POST_AND_COMMENTS_FILE_HEADERS

                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerows([{'PostText': encode_text(PostText), 'PosterName': encode_text(PosterName), 'PostDate': LastCommentPostDate,'CommentDate': "",
                ...
                with open('PostUpdates.csv', 'a') as csvfile:
                    ...
                    writer.writerows([{'WallPostId': convert(wallPostIdFormatted), 'PostDate': LastCommentPostDate, 'LinkToPost': PostLink}])
        ...
        if AllCommentsforPostElements:
            ...
            for CommentBlock in AllCommentsforPostElements:
                ...
                FormattedWallPostUniqueCommentActorXpathWithId = WallPostUniqueCommentActorXpathWithId.format(wallPostId,CommentBlockId)
                FormattedWallPostUniqueCommentTextXpathWithId = WallPostUniqueCommentTextXpathWithId.format(wallPostId,CommentBlockId)
                FormattedWallPostUniqueTimestampXpathWithId = WallPostUniqueTimestampXpathWithId.format(wallPostId,CommentBlockId)
                ...
                commentAsListToWrite = [{'PostText': "", 'PosterName': "", 'PostDate': "", 'CommentDate' : MyCommentDate,
                                         'CommentContact': encode_text(myCommentActor), 'CommentDetails': encode_text(MyCommentText), 'CommentMedia': CommentImageFileName}]
                savetoCsv("Posts\\", P_formattedLastCommentPostDate + "_" + FormattedwallPostID, POST_AND_COMMENTS_FILE_HEADERS, commentAsListToWrite, "Comment contents of " + FormattedwallPostID + " is saved.")
except:
    print(*"")
